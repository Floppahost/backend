...
func GetUpload(uploadId string) (map[string]any, error) {
	db := DB
	result := map[string]any{}
	db.Raw("SELECT mime_type, file_url, uploads.created_at, color, upload_id, user_id, title, site_name, site_name_url, description, author, author_url, file_name, username FROM uploads INNER JOIN users ON uploads.user_id = users.id WHERE deleted_at IS NULL AND path = ?", uploadId).Find(&result)
	if len(result) <= 0 {
